...
public class TeacherHandler {
  ...
  private void printTeacher(StreamTool streamTool) throws Exception {
    ...
    streamTool.printf("    이름: %s\n", m.getName()).printf("    전화: %s\n", m.getTel())
        .printf("  이메일: %s\n", m.getEmail()).printf("    학위: %s\n", getDegreeText(m.getDegree()))
        .printf("    학교: %s\n", m.getSchool()).printf("    전공: %s\n", m.getMajor())
        .printf("  강의료: %s\n", m.getWage()).printf("  등록일: %s\n", m.getCreatedDate()).send();
  }
  ...
  private void modifyTeacher(StreamTool streamTool) throws Exception {
    ...
    m.setDegree(streamTool.promptInt(String.format(
        "1. 고졸\n2. 전문학사\n3. 학사\n4. 석사\n5. 박사\n0. 기타\n학위(%s)? ", getDegreeText(old.getDegree()))));
      ...
      try {
        switch (menuNo) {
          case 0:
            ...
            return;
          case 1:
            this.inputTeacher(streamTool);
            break;
          case 2:
            this.printTeachers(streamTool);
            break;
          case 3:
            this.printTeacher(streamTool);
            break;
          case 4:
            this.modifyTeacher(streamTool);
            break;
          case 5:
            this.deleteTeacher(streamTool);
            break;
          default:
